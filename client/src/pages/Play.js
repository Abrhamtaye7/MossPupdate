...
const Play = ({ history }) => {
  ...
  useEffect(() => {
    !socket &&
      openModal(
        () => (
          <Text>{getLocalizedString('Lost Connection')}</Text>
        ),
        getLocalizedString('You lost connection to the server'),
        getLocalizedString('Back to Lobby'),
        () => history.push('/'),
      );
    socket && joinTable(1);
    return () => leaveTable();
          ...
          <>
            ...
            >
              <Button small secondary onClick={leaveTable}>
                {getLocalizedString('Leave Table')}
              </Button>
            </PositionedUISlot>
            {!isPlayerSeated && (
              <PositionedUISlot
