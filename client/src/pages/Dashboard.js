    ...
    <RelativeWrapper>
      ...
      >
        <Form onSubmit={(e) => e.preventDefault()}>
          ...
          <Wrapper>
            <FormGroup>
              <Label>{getLocalizedString('Username')}</Label>
              <Input value={userName} />
              <Button primary>
                {getLocalizedString('Edit username')}
              </Button>
            </FormGroup>
            <FormGroup>
              <Label>{getLocalizedString('Email')}</Label>
              <Input type="email" value={email} />
              <Button primary>
                {getLocalizedString('Edit email')}
              </Button>
            </FormGroup>
            <FormGroup style={{ gridColumnStart: '1', gridColumnEnd: '3' }}>
              <Button primary>
                {getLocalizedString('Reset password')}
              </Button>
              <Button>
                {getLocalizedString('Delete account')}
              </Button>
            </FormGroup>
            <Button
              as={Link}
